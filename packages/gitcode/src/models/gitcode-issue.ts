...
export class GitCodeIssue extends Issue {
  ...
  get repository(): string {
    const url = (this.data.repository_url as string) ?? (this.data as any).html_url as string ?? "";
    return url?.replace(/https?:\/\/[^/]+\/api\/v5\/repos\//, "").replace(/https?:\/\/[^/]+\//, "").replace(/\/issues\/\d+$/, "") ?? "";
  }
  get author(): User { return mapUser(this.data.user); }
  get assignees(): User[] { return ((this.data.assignees as any[]) ?? []).map(mapUser); }
  get labels(): Label[] {
    return ((this.data.labels as any[]) ?? []).map((l: any) => ({ id: String(l.id ?? l.name), name: l.name, color: l.color ?? "000000", description: l.description ?? null }));
  }
  get rawData() { return this.data; }

  async close(): Promise<void> { await this.http.request("PATCH", `/repos/${this.repository}/issues/${this.iid}`, { body: JSON.stringify({ state: "closed" }) }); }
  async reopen(): Promise<void> { await this.http.request("PATCH", `/repos/${this.repository}/issues/${this.iid}`, { body: JSON.stringify({ state: "open" }) }); }
  async update(changes: IssueUpdates): Promise<this> { await this.http.request("PATCH", `/repos/${this.repository}/issues/${this.iid}`, { body: JSON.stringify(changes) }); return this; }
  async addLabel(label: string): Promise<void> { await this.http.request("POST", `/repos/${this.repository}/issues/${this.iid}/labels`, { body: JSON.stringify({ labels: [label] }) }); }
  async removeLabel(_label: string): Promise<void> { /* GitCode: DELETE /issues/{number}/labels removes ALL labels */ }
  ...
  async createNote(body: string): Promise<IssueNote> {
    const result = await this.http.request<Record<string, unknown>>("POST", `/repos/${this.repository}/issues/${this.iid}/comments`, { body: JSON.stringify({ body }) });
