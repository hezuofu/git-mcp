...
export class GiteeApiError extends PlatformApiError {
  constructor(statusCode: number, body: any) {
    super(statusCode, body?.message ?? body?.error_description ?? "Unknown Gitee API error", body);
  }
  ...
  map(): DomainError {
    ...
    switch (this.statusCode) {
      ...
      case 422: return new ValidationError(JSON.stringify(this.body), this);
