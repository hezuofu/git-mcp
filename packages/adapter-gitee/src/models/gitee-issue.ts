...
export class GiteeIssue extends Issue {
  ...
  get repository(): string { return (this.data.repository as any)?.full_name as string ?? ""; }
  ...
  async removeLabel(label: string): Promise<void> { await this.http.request("DELETE", `/repos/${this.repository}/issues/${this.iid}/labels/${encodeURIComponent(label)}`); }
