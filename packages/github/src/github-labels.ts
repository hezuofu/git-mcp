...
export class GitHubLabelCollection implements ILabelCollection {
  constructor(private readonly http: GitHubHttpClient) {}
  async list(repo: string): Promise<PaginatedList<Label>> {
    const data = await this.http.request<Record<string, unknown>[]>("GET", `/repos/${repo}/labels?per_page=100`);
  ...
  async get(repo: string, name: string): Promise<Label> {
    const l = await this.http.request<any>("GET", `/repos/${repo}/labels/${encodeURIComponent(name)}`);
  ...
  async create(repo: string, params: { name: string; color: string; description?: string }): Promise<Label> {
    const l = await this.http.request<any>("POST", `/repos/${repo}/labels`, { body: JSON.stringify(params) });
    return { id: String(l.id), name: l.name, color: l.color, description: l.description ?? null };
  }
  async update(repo: string, name: string, params: { newName?: string; color?: string; description?: string }): Promise<Label> {
    const l = await this.http.request<any>("PATCH", `/repos/${repo}/labels/${encodeURIComponent(name)}`, { body: JSON.stringify({ new_name: params.newName, color: params.color, description: params.description }) });
    return { id: String(l.id), name: l.name, color: l.color, description: l.description ?? null };
  }
  async delete(repo: string, name: string): Promise<void> {
    await this.http.request("DELETE", `/repos/${repo}/labels/${encodeURIComponent(name)}`);
  }
}

export class GitHubLabelProvider implements ToolProvider {
  constructor(private readonly c: GitHubLabelCollection) {}
