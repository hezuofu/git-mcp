...
export class GitLabHttpClient extends PlatformHttpClient {
  ...
  async request<T>(method: string, path: string, options?: RequestOptions): Promise<T> {
    ...
    if (!response.ok) throw this.buildApiError(response.status, body, Object.fromEntries(response.headers.entries()));
    return body as T;
  }

  protected buildApiError(statusCode: number, body: any, headers?: Record<string, string>): Error {
